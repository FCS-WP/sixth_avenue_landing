...
add_action('after_setup_theme', 'themo_themo_setup');
 
function themo_themo_setup() {

    // Make theme available for translation
    // Get the locale
    $locale = apply_filters('theme_locale', get_locale(), 'uplands');
    // Try and load the user generated .mo outside of the theme directory.
    // It's name convention is uplands-en_US.mo
    load_textdomain('uplands', WP_LANG_DIR.'/uplands/'.'uplands'.'-'.$locale.'.mo');
    // Last, load our default (if we have one). Name convetion is just en_US.mo (not including theme name).
    load_theme_textdomain('uplands', get_template_directory() . '/languages');


	// Register wp_nav_menu() menus (http://codex.wordpress.org/Function_Reference/register_nav_menus)
	register_nav_menus(array(
	'primary_navigation' => esc_html__('Primary Navigation', 'uplands'),
	));
	...
	if ( function_exists( 'add_image_size' ) ) { 
		...
		if ( function_exists( 'get_theme_mod' ) ) {
			$logo_height = get_theme_mod( 'themo_logo_height', 30 );
