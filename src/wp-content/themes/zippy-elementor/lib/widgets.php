...
function roots_widgets_init() {
	...
	register_sidebar(array(
		'name'          => esc_html__('Primary', 'uplands'),
		'id'            => 'sidebar-primary',
	...
	if ( function_exists( 'get_theme_mod' ) ) {
		...
		if($themo_footer_show == true){
			...
			for ($i = 1; $i <= $themo_footer_columns; $i++) {
				register_sidebar(array(
					'name'          => sprintf(esc_html__('Upper Footer Column %1$s', 'uplands'),$i),
					'id'            => "sidebar-footer-$i",
					'before_widget' => '<section class="widget %1$s %2$s"><div class="widget-inner">',
					'after_widget'  => '</div></section>',
					'before_title'  => '<h3 class="widget-title">',
        ...
        if($themo_footer2_show == true){
            ...
            for ($i = 1; $i <= $themo_footer2_columns; $i++) {
                register_sidebar(array(
                    'name'          => sprintf(esc_html__('Lower Footer Column %1$s', 'uplands'),$i),
...
class WP_Widget_Themo_Social_Icons extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_themo_social_icons', 'description' => esc_html__( "Social Icons", 'uplands') );
		parent::__construct('themo-social-icons', esc_html__('Social Icons', 'uplands'), $widget_ops);
	...
	function form( $instance ) {
        ...
        $defaults = array( 'title' => __( 'Be Social', 'uplands' ), 'themo_social_align' => 'off' );
...
?>
        ...
        <!-- The checkbox -->
        <p>
            <input class="checkbox" type="checkbox" <?php checked( $instance[ 'themo_social_align' ], 'on' ); ?> id="<?php echo esc_attr($this->get_field_id( 'themo_social_align' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'themo_social_align' )) ?>" />
            <label for="<?php echo esc_attr($this->get_field_id( 'themo_social_align' )); ?>">Align Right</label>
        </p>
...
class WP_Widget_Themo_Payments_Accepted extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_themo_payments_accepted', 'description' => esc_html__( "Payments Accepted", 'uplands') );
		parent::__construct('themo-payments-accepted', esc_html__('Payments Accepted', 'uplands'), $widget_ops);
	...
	function form( $instance ) {
		$title = isset($instance['title']) ? esc_attr($instance['title']) : '';
		?>
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'uplands'); ?></label>
...
class WP_Widget_Themo_Contact_Info extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_themo_contact_info', 'description' => esc_html__( "Contact Info", 'uplands') );
		parent::__construct('themo-contact-info', esc_html__('Contact Info', 'uplands'), $widget_ops);
...
class WP_Widget_Themo_Logo extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'widget_themo_logo', 'description' => esc_html__( "Footer Logo", 'uplands') );
        parent::__construct('themo-logo', esc_html__('Footer Logo', 'uplands'), $widget_ops);
