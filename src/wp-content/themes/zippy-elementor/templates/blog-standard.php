...
<div class="inner-container<?php echo esc_attr($th_no_sidebar_class); ?>">
    ...
    if (themo_empty_content($post->post_content)) {
    ...
    <div class="<?php echo sanitize_text_field($masonry_row_class); ?> row">
		<?php if (!have_posts()) : ?>
            <div class="alert">
            <?php esc_html_e('Sorry, no results were found.', 'uplands'); ?>
            </div>
            <?php get_search_form(); ?>
        <?php endif; ?>
...
themo_bittersweet_pagination();
                    }else{ ?>
                    <li class="previous"><?php next_posts_link(esc_html__('&larr; Older posts', 'uplands')); ?></li>
