    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function admin_menus() {
            ...
            if ( $this->is_submenu_page() ) {
                ...
                $add_subpage_function( $this->parent_slug, esc_html__( 'Theme Setup', 'uplands' ), esc_html__( 'Theme Setup', 'uplands' ), 'manage_options', $this->page_slug, array(
                    $this,
                    'setup_wizard',
                ) );
            } else {
                add_theme_page( esc_html__( 'Theme Setup', 'uplands' ), esc_html__( 'Theme Setup', 'uplands' ), 'manage_options', $this->page_slug, array(
                    $this,
                    'setup_wizard',
                ) );
            }
        ...
        public function init_wizard_steps() {
            ...
            $this->steps = array(
                'introduction' => array(
                    'name'    => esc_html__( 'Introduction', 'uplands' ),
            ...
            if( $this->should_we_install_child_theme() ) {
                $this->steps['child_theme'] = array(
                    'name' => esc_html__('Customization Support', 'uplands'),
...
//            }
        ...
        public function create_child_theme() {
            ...
            if ( wp_mkdir_p( $child_dir ) ) {
                ...
                wp_die( esc_html__( 'Error: theme folder not writable. Please install Child Theme manually or just go back and skip this step.', 'uplands' ) );
            }
        }
...
";
        ...
        public function setup_wizard() {
            ...
            wp_localize_script( 'envato-setup', 'envato_setup_params', array(
                ...
                'verify_text'      => esc_html__( '...verifying', 'uplands' ),
            ) );
    ...
    public function setup_wizard_header() {
        ...
        <head>
            ...
            echo '<t';
            echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Guide', 'uplands' ) . '</ti' . 'tle>'; ?>
        ...
        <?php if ( 'next_steps' === $this->step ) : ?>
            <a class="wc-return-to-dashboard"
               href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'uplands'); ?></a>
        <?php endif; ?>
        </body>
        <?php
        ...
        public function envato_setup_introduction() {
            ...
            if ( false && isset( $_REQUEST['debug'] ) ) {
                ...
                ?>
                <h1><?php printf( esc_html__( 'Upgrade?', 'uplands' ), wp_get_theme() ); ?></h1>
                <p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup guide will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'uplands' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'uplands' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button button-large"><?php esc_html_e( 'Not right now', 'uplands' ); ?></a>
                </p>
                <?php
            } else if ( get_option( 'envato_setup_complete', false ) ) {
                ?>
                <h1><?php printf( esc_html__( 'Are you sure? It looks like you have already run the setup guide.', 'uplands' ), wp_get_theme() ); ?></h1>
                <p><?php _e( 'If you would like to switch demo presets and start from a clean install, please use the <a href="https://wordpress.org/plugins/wordpress-reset/" target="_blank">WordPress Reset plugin</a> before rerunning the Setup Guide. This will remove all content and reset your WordPress install to its factory settings. ', 'uplands' ); ?></p>
                <p><?php esc_html_e( 'If you wish to rerun the Setup Guide, please make sure to select the same preset demo you selected in the original run.', 'uplands' ); ?></p>
                <p><?php esc_html_e( 'Are you sure you want to run it again?', 'uplands' ); ?></p>
                <ul class="no-style">
                    <li>
                        <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                           class=" button button-next button-large"><?php esc_html_e( 'Run Setup Guide Again', 'uplands' ); ?></a>
                    </li>
                    <li>
                        <form method="post">
                            <input type="hidden" name="reset-font-defaults" value="yes">
                            <!--input type="submit" class="button-primary button button-large button-next"
                                   value="<?php esc_attr_e( 'Reset font style and colors', 'uplands' ); ?>" name="save_step"/-->
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button-primary button button-large"><?php esc_html_e( 'Cancel', 'uplands' ); ?></a>
                </p>
                <?php
            } elseif(isset($_REQUEST['page']) && $_REQUEST['page'] ==  'uplandschildtheme-setup') {
                ?>
                <h1><?php printf( esc_html__( 'Nice work! Ready to continue?', 'uplands' ), wp_get_theme() ); ?></h1>
                <p><?php printf( esc_html__( 'Let\'s start installing a few plugins that help make this theme run smoothly.', 'uplands' ), wp_get_theme() ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'uplands' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                ...
                ?>
                <h1><?php printf( esc_html__( 'Thank you for choosing Uplands! Ready to get setup?', 'uplands' ), wp_get_theme() ); ?></h1>
                <p><?php printf( esc_html__( 'To make your life easier, we have provided you with a 3 minute step by step installer. It is the best place to get started with the theme setup. Click "Let\'s Go" to continue. ', 'uplands' ), wp_get_theme() ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'uplands' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
        ...
        public function envato_setup_introduction_save() {
            ...
            if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'uplands' ); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        public function envato_setup_default_plugins() {
            ...
            ?>
            <h1><?php esc_html_e( 'Theme Plugins', 'uplands' ); ?></h1>
            <form method="post">
                ...
                if ( count( $plugins['all'] ) ) {
                    ?>
                    <p><?php esc_html_e( 'Your website needs a few essential plugins. Don\'t worry, you can add and remove plugins later on from within WordPress. The following plugins need to be installed or updated:', 'uplands' ); ?></p>
                    ...
                    echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'uplands' ) . '</strong></p>';
                } ?>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       ...
                       data-callback="install_plugins"><?php esc_html_e( 'Continue' , 'uplands'); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step' , 'uplands'); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
            </form>
            <?php
        ...
        public function ajax_plugins() {
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'uplands' ) ) );
            }
            $json = array();
            ...
            foreach ( $plugins['activate'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Activating Plugin', 'uplands' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['update'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Updating Plugin', 'uplands' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['install'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Installing Plugin', 'uplands' ),
                    );
                    break;
                }
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','uplands' ) ) );
            }
            exit;
        ...
        private function _content_default_get() {
            ...
            foreach ( $available_content as $post_type => $post_data ) {
                if ( count( $post_data ) ) {
                    ...
                    $content[ $post_type ] = array(
                        ...
                        'description'      => sprintf( esc_html__( 'Create default %s as seen on the demo.', 'uplands' ), $post_type_title ),
                        'pending'          => esc_html__( 'Pending.', 'uplands' ),
                        'installing'       => esc_html__( 'Installing.', 'uplands' ),
                        'success'          => esc_html__( 'Success.', 'uplands' ),
            ...
            $content['widgets'] = array(
                'title'            => esc_html__( 'Widgets', 'uplands' ),
                'description'      => esc_html__( 'Insert default sidebar widgets as seen on the demo.', 'uplands' ),
                'pending'          => esc_html__( 'Pending.', 'uplands' ),
                'installing'       => esc_html__( 'Installing Default Widgets.', 'uplands' ),
                'success'          => esc_html__( 'Success.', 'uplands' ),
            ...
            $content['settings'] = array(
                'title'            => esc_html__( 'Settings', 'uplands' ),
                'description'      => esc_html__( 'Configure default settings.', 'uplands' ),
                'pending'          => esc_html__( 'Pending.', 'uplands' ),
                'installing'       => esc_html__( 'Installing Default Settings.', 'uplands' ),
                'success'          => esc_html__( 'Success.', 'uplands' ),
            ...
            $content['forms'] = array(
                'title'            => esc_html__( 'Forms', 'uplands' ),
                'description'      => esc_html__( 'Add Default Forms.', 'uplands' ),
                'pending'          => esc_html__( 'Pending.', 'uplands' ),
                'installing'       => esc_html__( 'Adding Default Forms.', 'uplands' ),
                'success'          => esc_html__( 'Success.', 'uplands' ),
        ...
        public function envato_setup_default_content() {
            ?>
            <h1><?php esc_html_e( 'Demo Content', 'uplands' ); ?></h1>
            <form method="post">
                <?php if ( $this->is_possible_upgrade() ) { ?>
                    <p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'uplands' ); ?></p>
                <?php } else { ?>
                    <p><?php printf( esc_html__( 'Getting started is easier with demo content. We recommend checking all boxes as you can always choose to remove content from within WordPress after setup.','uplands' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
                ...
                <table class="envato-setup-pages" cellspacing="0">
                    ...
                    <tr>
                        <td class="check"></td>
                        <th class="item"><?php esc_html_e( 'Item', 'uplands' ); ?></th>
                        <th class="description"><?php esc_html_e( 'Description', 'uplands' ); ?></th>
                        <th class="status"><?php esc_html_e( 'Status', 'uplands' ); ?></th>
                    </tr>
                    </thead>
                    <tbody>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"
                       data-callback="install_content"><?php esc_html_e( 'Continue', 'uplands' ); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'uplands' ); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
        ...
        public function ajax_content() {
            ...
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'uplands' ) ) );
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array(
                    'error'   => 1,
                    'message' => esc_html__( 'Error', 'uplands' ),
                    'logs'    => $this->logs,
                    'errors'  => $this->errors,
                ) );
            }
                               ...
                               //if($post_id == 29 || $post_id == 20){
                                ...
                                if ('_elementor_edit_mode' == $meta_key && 'builder' === $meta_val){
                                    ...
                                    if(!isset($elm_page_settings_array) || empty($elm_page_settings_array)) {
                                        ...
                                        $group1 = array("home-cta","home-cta","home-form","home-slider","home-calendar",
                                            "home-video","home-masonry-blog","home-shop","coming-soon",
                                            "blog-masonry","blog-masonry-full-width","blog-masonry-sidebar",
                                            "blog-standard","blog-standard-sidebar","careers","cart","checkout",
                                            "contact","contact-2","course-guide-3-column","course-guide-4-column",
                                            "course-guide-5-column","faq","fees","gallery","my-account","our-location",
                                            "shop","shop-2","about-us","academy","events","membership","restaurant");
                                        ...
                                        if (in_array($th_slug, $group1)) {
                                            $elm_page_settings_array = array ('hide_title'=>'yes','themo_transparent_header'=>'','themo_header_hide_shadow'=>'yes');
        ...
        private function _fetch_remote_file( $url, $post ) {
            ...
            if ( ! $upload || $upload['error'] ) {
                ...
                if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'uplands' ) );
                }
                ...
                if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'uplands' ) );
                }

                if ( 0 == $filesize ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'uplands' ) );
                }

                if ( ! empty( $max_size ) && $filesize > $max_size ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'uplands' ), size_format( $max_size ) ) );
                }
            }
        ...
        public function _content_install_settings() {
            ...
            switch (get_theme_mod('dtbwp_site_style',$this->get_default_theme_style())){
                ...
                case 'style1':
                    // GOLF COURSE
                    $homepage = get_page_by_title( 'Home - CTA' );
                    break;
        ...
        public function envato_setup_child() {
            ?>
            <form method="post">
                <h1><?php esc_html_e( 'Customization Support', 'uplands' ); ?></h1>
                ...
                <p class="envato-setup-actions step">
                    <input type="submit" class="button-primary button button-large button-next"
                           value="<?php esc_attr_e( 'Continue', 'uplands' ); ?>" name="save_step"/>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this', 'uplands' ); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
            </form>
            <?php
        ...
        public function envato_setup_color_style() {
            ...
            ?>
            <h1><?php esc_html_e( 'Theme Demo Style', 'uplands' ); ?></h1>
            <form method="post">
                <p><?php esc_html_e( 'Which demo would you like to install? You can always customize your page layout and style settings from within WordPress after setup. Choose your favorite and click continue. ', 'uplands' ); ?></p>
...
//            wp_redirect( esc_url_raw( $this->get_next_step_link() ) );
        ...
        public function envato_setup_logo_design() {
            ...
            ?>
            <h1><?php esc_html_e( 'Logo', 'uplands' ); ?></h1>
            <form method="post">
                <p><?php esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.','uplands' ); ?></p>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            <a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'uplands' ); ?></a>
                        </td>
                    </tr>
                </table>
        ...
        public function envato_setup_updates() {
            ?>
            <h1><?php esc_html_e( 'Theme Updates', 'uplands' ); ?></h1>
            <?php if ( function_exists( 'envato_market' ) ) { ?>
                <form method="post">
                    ...
                    if ( count( $my_items ) ) {
                        ...
                        <p class="envato-setup-actions step">
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                               class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'uplands'); ?></a>
                        </p>
                        <?php
                    } else {
                        ?>
                        <p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'uplands' ); ?></p>
                        <p>When an update becomes available it will show in the Dashboard with an option to install.</p>
                        <p>
                        ...
                        <p class="envato-setup-actions step">
                            <input type="submit" class="button-primary button button-large button-next"
                                   value="<?php esc_attr_e( 'Login with Envato', 'uplands' ); ?>" name="save_step"/>
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                               class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'uplands' ); ?></a>
                            <?php wp_nonce_field( 'envato-setup' ); ?>
                        </p>
        ...
        public function envato_setup_customize() {
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'uplands' ); ?></a>
            </p>
        ...
        public function envato_setup_help_support() {
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'uplands' ); ?></a>
                <?php wp_nonce_field( 'envato-setup' ); ?>
            </p>
            <?php
        }
        ...
        public function envato_setup_ready() {
            ...
            <a href="https://twitter.com/share" class="twitter-share-button"
               ...
               data-text="<?php echo esc_attr( 'I just setup my new website using the Uplands #WordPress theme from #ThemeForest' ); ?>"
            ...
            <h1><?php esc_html_e( 'Your website is ready!', 'uplands' ); ?></h1>
            ...
            <div class="envato-setup-next-steps">
                <div class="envato-setup-next-steps-first">
                    <h2><?php esc_html_e( 'Next steps', 'uplands' ); ?></h2>
                    <ul>
                        <li class="setup-product"><a class="button button-primary button-large"
                                                     href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'uplands' ); ?></a>
                        </li>
                        <li class="setup-product"><a class="button button-next button-large"
                                                     href="https://twitter.com/themovation"
                                                     target="_blank"><?php esc_html_e( 'Follow @themovation on Twitter', 'uplands' ); ?></a>
                        </li>
                    </ul>
                </div>
                <div class="envato-setup-next-steps-last">
                    <h2><?php esc_html_e( 'Resources', 'uplands' ); ?></h2>
                    <ul>
                        <li class="documentation"><a href="http://themovation.helpscoutdocs.com/"
                                                     target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'uplands' ); ?></a>
                        </li>
                        <li class="support"><a href="https://themovation.ticksy.com/"
                                               target="_blank"><?php esc_html_e( 'Get Help and Support', 'uplands' ); ?></a></li>
                    </ul>
                </div>
            </div>
            <?php
        ...
        public function envato_market_admin_init() {
            ...
            if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
                ...
                if ( is_wp_error( $response ) ) {
                    ...
                    $class         = 'error';
                    echo "<div class='".esc_attr($class)."'><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'uplands' ), $error_message ) . '</p></div>';
                } else {
            ...
            add_settings_section(
                ...
                sprintf( esc_html__( 'Login for %s updates', 'uplands' ), $this->envato_username ),
            ...
            add_settings_field(
                ...
                esc_html__( 'oAuth Login', 'uplands' ),
                ...
                if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
                    ...
                    if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            <p><?php
                                _e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.','uplands' );
                                ?></p>
                            <p>
                                <?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>','uplands' ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
                            </p>
                        </div>
