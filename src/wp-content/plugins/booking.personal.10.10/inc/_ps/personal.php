...
if ( ! defined( 'IS_USE_WPDEV_BK_CACHE' ) ) { define( 'IS_USE_WPDEV_BK_CACHE', true ); }								// FixIn: 9.9.0.13.

require_once(WPBC_PLUGIN_DIR. '/inc/_ps/resource_property/resource_property.php' );  									// Resources Meta - Properties of booking resources 	// FixIn: 9.9.0.13.
...
require_once(WPBC_PLUGIN_DIR. '/inc/_ps/class/wpbc-settings-table.php' );       										// Abstract Settings Table Class
...
require_once(WPBC_PLUGIN_DIR. '/inc/_ps/flex_table/wpbc_flextable.php' );       										// Abstract Settings FlexTable Class				// FixIn: 9.9.0.7.
require_once(WPBC_PLUGIN_DIR. '/inc/_ps/flex_table/wpbc_flextable_resources.php' );       								// Class for showing FlexTable of booking resources // FixIn: 9.9.0.7.
require_once(WPBC_PLUGIN_DIR. '/inc/_ps/flex_table/wpbc_flextable__reused_ui.php' );       								// Common reused UI elements in Flex Tables			// FixIn: 10.0.0.17.

require_once(WPBC_PLUGIN_DIR. '/inc/_ps/lib_p.php' );
require_once(WPBC_PLUGIN_DIR. '/inc/_ps/p-toolbar.php' );

require_once(WPBC_PLUGIN_DIR. '/inc/_ps/wpbc-booking-select-widget.php' );												// FixIn: 9.6.3.5.
...
//require_once(WPBC_PLUGIN_DIR. '/inc/_ps/includes/booking_form/form_templates.php' );									// Booking Form  Templates // FixIn: 10.6.2.1.


require_once( WPBC_PLUGIN_DIR . '/inc/_ps/includes/booking_form/flex_toolbar__booking_form.php' );                      // Toolbar for Booking Form
...
require_once( WPBC_PLUGIN_DIR . '/inc/_ps/admin/page-settings-up.php' );
...
if ( is_admin() ) {
	...
	if ( file_exists( WPBC_PLUGIN_DIR . '/inc/_ps/wpbc-check-updates.php' ) ) {            // Checking updates
		...
		$wpbc_plugin_updater = new WPBC_PlUpdater( WPBC_FILE, 'https://wpbookingcalendar.com/check-update/booking.json', array( 'version'        => WP_BK_VERSION_NUM,
...
class wpdev_bk_personal   {
	...
	function __construct() {
		...
		add_bk_action( 'booking_pending_send_email', array( $this, 'booking_aproved_afteraction' ) );  // FixIn: 10.9.6.1.
		add_bk_action( 'booking_trash_send_email', array( $this, 'booking_aproved_afteraction' ) );
...
//     H   A   S   H                          //HASH_EDIT /////////////////////////////////////////////////////////////////////////////////////////
    ...
    function set_booking_edit_link_at_email($mail_body,$booking_id ){
                ...
                if ($my_booking_id_type !== false) {
                    ...
                    //if ($my_boook_type == '') return '<strong>' . esc_html__('Oops!' ,'booking') . '</strong> ' . __('We could not find your booking. The link you used may be incorrect or has expired. If you need assistance, please contact our support team.' ,'booking');
                } else {
	 ...
	 * @param string $booking_form_show - form.
	 *
	 * @return void
	 */
	public function booking_aproved_afteraction( $res, $booking_form_show ) {
		$is_change_hash_after_approvement = get_bk_option( 'booking_is_change_hash_after_approvement' );
		if ( 'On' === $is_change_hash_after_approvement ) {
    ...
    function delete_booking_by_visitor(){   global $wpdb;
        ...
        if ($my_booking_id_type !== false) {
            ...
            if ( ($my_boook_type_new == '') || ($my_boook_type_new == false) ) {
                ...
                ?>
                <script type="text/javascript">
                    document.getElementById('submiting<?php echo esc_attr( $my_boook_type ); ?>').innerHTML = '<div class=\"submiting_content\" ><?php echo '<strong>' . esc_js(__('Oops!' ,'booking') ). '</strong> ' . esc_js(__('We could not find your booking. The link you used may be incorrect or has expired. If you need assistance, please contact our support team.' ,'booking') ); ?></div>';
        ...
        if ( ($my_edited_bk_id !=false) && ($my_edited_bk_id !='')) {
            ...
            ?> <script type="text/javascript">
                document.getElementById('submiting<?php echo esc_attr( $in_page_booking_type ); ?>').innerHTML = '<div class=\"submiting_content\" ><div style=&quot;height:20px;width:100%;text-align:center;margin:15px auto;&quot;><?php echo esc_js(__('The booking has been canceled successfully' ,'booking')); ?></div></div>';
...
// C l i e n t     s i d e     f u n c t i o n s     /////////////////////////////////////////////////////////////////////////////////////////
    ...
    function wpbc_enqueue_js_files( $where_to_load = 'both' ){
    	...
    	if ( ! wp_script_is( 'gform_masked_input' ) ) {
        	// FixIn: 8.7.9.10.
        	wp_enqueue_script( 'wpbc-meio-mask', WPBC_PLUGIN_URL . '/inc/js/meiomask.js', array( 'wpbc_all' ), WP_BK_VERSION_NUM, array( 'in_footer' => WPBC_JS_IN_FOOTER ) );
        }
        wp_enqueue_script( 'wpbc-personal',  WPBC_PLUGIN_URL . '/inc/js/personal.js', array( 'wpbc_all' ), WP_BK_VERSION_NUM, array( 'in_footer' => WPBC_JS_IN_FOOTER ) );
    }
...
// P A R S E   F o r m                      //////////////////////////////////////////////////////////////////////////////////////////////////
    function get_booking_form($my_boook_type, $my_booking_form = 'standard', $custom_params = array() ){    // FixIn: 6.0.1.5.
        ...
        if ( $this->current_edit_booking !==  false ) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
            if (       ( ! isset( $_GET['resource_no_update'] ) )                                   					// FixIn: 9.4.2.3.
    ...
    function get_booking_data($booking_id){
        ...
        return $return;
			...
			//FixIn: 9.9.0.12	-	Added [selectbox name "option"]  field
            ...
            function form_element_replace_callback($matches) {
	            ...
	            if ( isset( $this->processing_unit_tag ) ) {
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
					if ( isset( $_POST['wpdev_unit_tag'] ) && ( $this->processing_unit_tag === $_POST['wpdev_unit_tag'] ) ) {
            ...
            if ($class_att)
	            ...
	            // Value.
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
				if ( ( isset( $_POST['wpdev_unit_tag'] ) ) && ( isset( $this->processing_unit_tag ) ) && ( $this->processing_unit_tag === $_POST['wpdev_unit_tag'] ) ) {
		            // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	            ...
	            if ( preg_match( '/^(?:select|selectbox|checkbox|radio)[*]?$/', $type ) ) {
	            ...
	            switch ( $type ) {
		            ...
		            case 'captchar':
		            	...
		            	// FixIn: 10.8.1.2.
		               	$field_type = ( ( $type == 'email' ) || ( $type == 'email*' ) )
						  			? 'type="email"'
						  			: 'type="text"';
		            ...
		            case 'select*':
			            ...
			            if (
							   ( preg_match( '/^select[*]?$/', $type ) && $multiple && ( $name == 'rangetime' . $this->current_booking_type ) )
							|| ( preg_match( '/^selectbox[*]?$/', $type ) && $multiple && ( $name == 'rangetime' . $this->current_booking_type ) )
						){
			            ...
			            foreach ( $values as $key => $value ) {
				            ...
				            if ( ( isset( $this->processing_unit_tag ) ) && ( $this->processing_unit_tag == $_POST['wpdev_unit_tag'] ) && ( $multiple && in_array( $value, (array) $_POST[ $name ] ) || ! $multiple && $_POST[ $name ] == $value ) ) {
					            $selected = ' selected="selected"';
				            }
		            ...
		            case 'radio*':
			            ...
			            foreach ( $values as $key => $value ) {
							...
							//TODO: ??? Need to retest this parameter.

							// phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
							if ( ( isset( $_POST['wpdev_unit_tag'] ) ) && ( isset( $this->processing_unit_tag ) ) && ( $this->processing_unit_tag === $_POST['wpdev_unit_tag'] )
					            && (
						            $multiple
						            && in_array( $value, (array) $_POST[ $name ] )		// phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
						            || ! $multiple
						               && $_POST[ $name ] == $value                     // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					            ) ) {
					            $checked = ' checked="checked"';
				            }
				            // -----------------------------------------------------------------------------------------
				            ...
				            if ( $is_use_label == 'label' ) {
					            ...
					            if ( count( $id_matches ) > 0 ) {
						            ...
						            $id_attr_for_checkbox = $id_matches[1] . time() . $key . wp_rand( 1000, 10000 );     //Uniq ID		// FixIn: 8.8.1.11.
					            } else {
						            $id_attr_for_checkbox = 'checkboxid' . time() . $key . wp_rand( 1000, 10000 );       //Uniq ID		// FixIn: 8.8.1.11.
					            }
				            ...
				            if ( $is_label_wrap ) {
					            ...
					            if ( $is_use_label_first ) {
						            //$item = $item_label . '&nbsp;' . $item;			// FixIn: 10.1.5.7.
						            $item = $item_label . $item;
					            } else {
						            $item = $item . $item_label;
						            //$item = $item . '&nbsp;' . $item_label;			// FixIn: 10.1.5.7.
					            }
				            }
		            ...
		            case 'captchac':
			            ...
			            $html        = '<img alt="To show CAPTCHA, please deactivate cache plugin or exclude this page from caching or disable CAPTCHA at WP Booking Calendar - Settings General page in Form Options section." src="' . $captcha_url . '"' . $atts . ' />';
				...
				if ( isset( $_GET['booking_hash'] ) ) {
					...
					if ( $my_booking_id_type !== false ) {
						...
						if ( isset( $_GET['booking_cancel'] ) ) {
							...
							$get_booking_hash = ( ( isset( $_GET['booking_hash'] ) ) ? sanitize_text_field( wp_unslash( $_GET['booking_hash'] ) ) : '' );  /* phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing */ /* FixIn: sanitize_unslash */
							$html       .= $wpbc_nonce . '<input type="button" value="' . $value . '"' . $atts . ' onclick="wpbc_customer_action__booking_cancel(\'' . $get_booking_hash . '\',' . $this->current_booking_type . ', \'' . wpbc_get_maybe_reloaded_booking_locale() . '\' );wpbc_booking_form__this_button__disable( this );" />';
							...
							if ( wpbc_is_new_booking_page() ) {    // FixIn: 8.4.5.9.
								...
								$html .= '<input type="button" value="' . __( 'Duplicate Booking', 'booking' ) . '"' . $atts . ' class="wpbc_button_light" style="margin:0 20px 0 0;"'
