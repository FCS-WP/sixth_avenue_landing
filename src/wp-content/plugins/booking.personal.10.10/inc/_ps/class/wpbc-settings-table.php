...
abstract class WPBC_Settings_Table {
    ...
    public function print_column_headers( $is_header = true ) {
        ...
        if ( $is_header ) {
            foreach ( $this->get_columns() as $col_id => $col_value ) {
                ...
                ?><th <?php echo   isset($col_value['id'] ) ? ' id="' . esc_attr( $col_value['id'] ). '" ' : ''; ?>
                        <?php echo isset($col_value['class'] ) ? ' class="' . esc_attr( $col_value['class'] ). '" ' : ''; ?>
                        <?php echo isset($col_value['style'] ) ? ' style="' . esc_attr( $col_value['style']) . '" ' : ''; ?>
                        <?php
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo isset($col_value['attr'] ) ? ' ' . $this->get_custom_attr_static( $col_value['attr'] ) . ' ' : ''; ?>
    ...
    public function display() {
        ...
        ?>
        <div class="wpdevelop wpbc_selectable_table <?php echo esc_attr( implode( ' ', $this->div_before_table_css_classes() ) ); ?>">
            <table class="table table-striped widefat <?php echo esc_attr( implode( ' ', $this->table_css_classes() ) ); ?>" cellspacing="0" cellpadding="0">
