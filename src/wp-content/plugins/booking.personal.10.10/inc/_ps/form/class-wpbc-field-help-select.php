...
class WPBC_Field_Help_Select extends WPBC_Field_Help_Text {
	...
	echo $this->update_js_function_name; ?>();"
                   ...
                   />
            <p class="help-block" style="white-space: nowrap;"><?php
				/* translators: 1: ... */
				echo wp_kses_post( sprintf( __( 'One Value from %1$sOptions%2$s list', 'booking' ),'<strong>','</strong>') ); ?></p>
        </div>
        <?php
    }
    ...
    public function setMultipleField($group_css=''){
        ...
        <div class="parameter-group <?php echo esc_attr( $group_css ); ?>" style="white-space: nowrap;">
            <input type="checkbox"  style="width: auto; padding: 0px; margin: 0 5px 0 0;"
                   id="<?php echo esc_attr( $this->htmlid ); ?>_multiple" name="<?php echo esc_attr( $this->htmlid ); ?>_multiple"
                   onchange="javascript:<?php
	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	echo $this->update_js_function_name; ?>();"
                   onclick="javascript:this.onchange();"
                   />
            <label class="" for="<?php echo esc_attr( $this->htmlid ); ?>_multiple" style="width: auto; display: inline;"><?php
            /* translators: 1: ... */
            echo wp_kses_post( sprintf( __( 'Allow %1$smultiple%2$s selections', 'booking' ),'<strong>','</strong>') ); ?>.</label>
        </div>
        <?php
    }
    ...
    public function js(){
        ...
        ?><script type="text/javascript">
			...
			echo $this->update_js_function_name; ?>(){
                ...
                if ( jQuery('#<?php echo esc_attr( $this->htmlid ); ?>_multiple').prop("checked") ) {
                    p_multiple = ' multiple';
                }
