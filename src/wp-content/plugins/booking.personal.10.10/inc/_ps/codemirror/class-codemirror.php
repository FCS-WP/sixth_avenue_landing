...
final class WPBC_CodeMirror {
	...
	public function wpbc_js_load_files( $where_to_load ) {
		...
		if ( ( is_admin() ) && ( in_array( $where_to_load, array( 'admin', 'both' ) ) ) ) {
		    ...
		    wp_localize_script( 'wpbc_all', 'wpbc_ce_settings', $wpbc_ce_settings );                            // FixIn: 8.7.8.6.
			...
			wp_enqueue_script( 'wpbc-codemirror'
							 , trailingslashit( plugins_url( '', __FILE__ ) ) . 'code_mirror.js'         /* wpbc_plugin_url( '/src/js/codemirror.js' ) */
	...
	public function wpbc_enqueue_css_files( $where_to_load ) {
		...
		if ( ( is_admin() ) && ( in_array( $where_to_load, array( 'admin', 'both' ) ) ) ) {
			...
			wp_enqueue_style( 'wpbc-codemirror'
							, trailingslashit( plugins_url( '', __FILE__ ) ) . 'code_mirror.css'         /* wpbc_plugin_url( '/src/css/code_mirror.css' ) */
		...
		<script type="text/javascript">
			jQuery( document ).ready( function (){
				...
				WPBC_CM.init_codemirror( {
											textarea_id : '<?php echo esc_attr( $params['textarea_id'] ); ?>'
											<?php
